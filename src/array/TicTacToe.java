...
class TicTacToe {
    ...
    public static void main(String[] args) {
        ...
        System.out.println("return " + ticTacToe.move(2,2,1));*/
        ...
        System.out.println("return " + ticTacToe.move(0,2,1));
    ...
    // this tracks how many rows , cols , diagnol a player has scored
    // if any of these is same size as board , the the player wins
    public int move(int row, int col, int player) {
        int toAdd = player == 1 ? 1 : -1;           // use 1 for player 1 and -1 for player 2
        rows[row] += toAdd;
        cols[col] += toAdd;
        if(row == col) diagonal += toAdd;
        if(row+col+1 == n) antiDiagonal += toAdd;
